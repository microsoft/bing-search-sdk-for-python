...
def image_search(subscription_key):
    ...
    try:
        ...
        if image_results.value:
            ...
            print(
                "First image thumbnail url: {}".format(first_image_result.thumbnail_url)
            )
        ...
        print(
            "Image result total estimated matches: {}".format(
        ...
        if image_results.pivot_suggestions:
            ...
            if first_pivot.suggestions:
                ...
                print(
                    "First suggestion web search url: {}".format(
        ...
        if image_results.query_expansions:
            ...
            print(
                "Query expansion count: {}".format(len(image_results.query_expansions))
            )
            print("First query expansion text: {}".format(first_query_expansion.text))
            print(
                "First query expansion search link: {}".format(
...
def image_search_with_filters(subscription_key):
    ...
    try:
        ...
        print(
            'Search images for "studio ghibli" results that are animated gifs and wide aspect'
        )
        ...
        if image_results.value:
            ...
            print(
                "First image web search url: {}".format(
...
def image_detail(subscription_key):
    ...
    try:
        ...
        image_detail = client.images.details(
            ...
            modules=[ImageInsightModule.all],  # Could be the str "all"
        )
        print(
            "Search detail for image insights token: {}".format(
                first_image.image_insights_token
            )
        )
        print(
            "Expected image insights token: {}".format(
        ...
        if image_detail.best_representative_query:
            print(
                "Best representative query text: {}".format(
                    image_detail.best_representative_query.text
                )
            )
            print(
                "Best representative query web search url: {}".format(
        ...
        if image_detail.image_caption:
            ...
            print(
                "Image caption data source url: {}".format(
        ...
        if image_detail.visually_similar_images:
            first_visually_similar_images = image_detail.visually_similar_images.value[
                0
            ]
            print(
                "Visually similar images count: {}".format(
                    len(image_detail.visually_similar_images.value)
                )
            )
            print(
                "First visually similar image name: {}".format(
                    first_visually_similar_images.name
                )
            )
            print(
                "First visually similar image content url: {}".format(
                    first_visually_similar_images.content_url
                )
            )
            print(
                "First visually similar image content size: {}".format(
...
if __name__ == "__main__":
    ...
    image_search(SUBSCRIPTION_KEY)
    image_search_with_filters(SUBSCRIPTION_KEY)
    image_trending(SUBSCRIPTION_KEY)
    image_detail(SUBSCRIPTION_KEY)
